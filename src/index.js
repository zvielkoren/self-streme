...
const STREAM_VALIDATION_TTL_MS =
  Number.parseInt(process.env.STREAM_VALIDATION_TTL_MS || "60000", 10) || 60000;
const STREAM_VALIDATION_TIMEOUT_MS =
  Number.parseInt(process.env.STREAM_VALIDATION_TIMEOUT_MS || "3000", 10) || 3000;
const STREAM_VALIDATION_CONCURRENCY = Math.min(
  5,
  Math.max(
    1,
...
function getCachedStreamValidation(url) {
  const cached = streamValidationCache.get(url);
  if (!cached) return null;

  if (cached.expiresAt <= Date.now()) {
    streamValidationCache.delete(url);
    return null;
  }
...
function setCachedStreamValidation(url, result) {
  streamValidationCache.set(url, {
    ...result,
    expiresAt: Date.now() + STREAM_VALIDATION_TTL_MS,
  });
}
...
async function validateSingleStreamUrl(url) {
  ...
  // Internal URLs are generated by this service and considered valid if shape is correct.
  if (trimmedUrl.startsWith("/")) {
    const result = { valid: true, reason: "internal-url", status: 200 };
  ...
  try {
    ...
    let result;
    if (!response.ok) {
      result = { valid: false, reason: `bad-status-${response.status}` };
    ...
    return { valid: false, reason: `request-error:${error.message}` };
  } finally {
...
async function filterAndValidateStreams(streams, context) {
  ...
  return {
    ...
    invalid: Math.max(0, candidates.length - workingStreams.length),
    rejected: Object.fromEntries(rejectionCounts),
  };
}
...
app.use((err, req, res, next) => {
  ...
  logger.error("[HTTP] Unhandled middleware error", {
    path: req.path,
    method: req.method,
...
app.get("/stream/:type/:imdbId.json", async (req, res) => {
  try {
    ...
    const rawStreams = await streamService.getStreams(
      type,
      cleanImdbId,
      season,
      episode,
    ...
    if (!Array.isArray(rawStreams)) {
      logger.warn(
        `[Stream] Non-array streams payload for ${type}:${cleanImdbId}; returning empty stream list`,
        { requestedId: imdbId, type, rawType: typeof rawStreams },
      );
    }
    const validated = await filterAndValidateStreams(streams, {
      type,
      imdbId: cleanImdbId,
      endpoint: "/stream/:type/:imdbId.json",
    });

    logger.info("[Stream] Validation summary (.json route)", {
      request: `${type}:${imdbId}`,
      totalFetched: validated.total,
    ...
    if (validated.streams.length === 0) {
      logger.info(
        `[Stream] Returning empty streams response for request ${type}:${imdbId}`,
      );
    }

    res.status(200).json({ streams: validated.streams });
  } catch (error) {
    logger.error("Stream endpoint error:", {
      requestedId: req.params.imdbId,
...
app.get("/stream/:type/:imdbId", async (req, res) => {
  try {
    ...
    const validated = await filterAndValidateStreams(streams, {
      type,
      imdbId: cleanImdbId,
      endpoint: "/stream/:type/:imdbId",
    });

    logger.info("[Stream] Validation summary", {
      request: `${type}:${imdbId}`,
      totalFetched: validated.total,
      candidates: validated.candidates,
      valid: validated.valid,
    ...
    if (validated.streams.length === 0) {
      logger.info(
        `[Stream] No valid streams found for ${type}:${imdbId}; returning {\"streams\":[]}`,
      );
    }

    res.status(200).json({ streams: validated.streams });
  } catch (error) {
    logger.error("Stream request error:", {
      requestedId: req.params.imdbId,
      type: req.params.type,
      message: error.message,
      stack: error.stack,
